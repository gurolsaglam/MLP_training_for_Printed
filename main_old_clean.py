...
if __name__ == "__main__":
    ...
    if (not os.path.exists(dataset_dump_folder)):
        ...
        #also add a excel sheet for result dumping.
        df = pd.DataFrame(columns=["dataset", "algo", "feature_range", "hidden_layer_sizes", "index", "joblib_filename", "params", "accuracy"])
    ...
    for i in range(0, 20):
        ...
        ml = algo(dataset.Xtrain, dataset.Xtest, dataset.Ytrain, dataset.Ytest, name, hidden_layer_sizes=hidden_layer_sizes)
        ...
        df.loc[len(df.index)] = [dataset_name, algo_name, feature_range, hidden_layer_sizes, str(i), dataset_dump_folder+name, params, acc]
        df.to_excel(results_dump_file, index=False)
