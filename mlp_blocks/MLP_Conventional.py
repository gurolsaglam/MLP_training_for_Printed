...
class MLP_Conventional:
    def __init__(self, weights, biases, fxp_inputs, fxp_qrelu, fxp_weights, fxp_biases,
        ...
        self.fixQA()
        self.interceptshifted = self.getShiftedIntercepts()
        self.QBshifted = self.getShiftedQB(biases, self.QB)
    ...
    def getShiftedIntercepts(self):
        ...
        for layer in range(len(self.coefs)):
            ...
            for i,neuron in enumerate(self.coefs[layer]):
                ...
                biases[layer].append(temp)
        return biases
    
    def getShiftedQB(self, intercept, QB):
        QBshifted = [b for b in QB]
        for layer in range(len(self.coefs)):
            product_M = self.QI[layer][1] + self.QW[layer][1]
            scale_bias = product_M - self.QB[layer][1]
            QBshifted[layer] = (QB[layer][0]+scale_bias, QB[layer][1])
        return QBshifted
    
    def fixQA(self):
        for layer in range(len(self.coefs)):
            sumprecisions = []
            for neuron in range(len(self.coefs[layer])):
                ...
                sumprecisions.append([swidth-decimal_s, decimal_s])
            #we got the sumprecisions for the layer, let's find the max lengths of integer part and decimal part
            sumprecisions = np.transpose(sumprecisions).tolist()
            max_precisions = [max(i) for i in sumprecisions]
            self.QA[layer] = (max_precisions[0], max_precisions[1])
            self.QI[layer+1] = (max_precisions[0], max_precisions[1])
    ...
    def write_neuron_comb(self, nprefix, layer, neuron, nmasks, activation, inputs, merge_list, sum_relu_size, lenMaxVal=None):
        ...
        numofWeights = len(self.coefs[0][0])*len(self.coefs[0])+len(self.coefs[1][0])*len(self.coefs[1])
        widthofWeights = sum(self.QW[0])
        numofBiases = len(self.intercept[0])+len(self.coefs[1])
        widthofBiases = [sum(self.QBshifted[0]), sum(self.QBshifted[1])]
        
        weightIndex = 0
        biasIndex = 0
        for i in range(0, layer+1):
            if (i == layer):
                for j in range(0, neuron):
                    weightIndex = weightIndex + widthofWeights * len(weights)
                    biasIndex = biasIndex + widthofBiases[i]
            else:
                for j in range(0, len(self.coefs[i])):
                    weightIndex = weightIndex + widthofWeights * len(self.coefs[i][0])
                    biasIndex = biasIndex + widthofBiases[i]
        ...
        for i in range(len(weights)):
            ...
            print("    wire signed [%d-1:0] %s;" % ((pwidth),name))
            
            print("    assign %s = $signed({1'b0, %s}) * $signed(%s);" % (name,a,"weights[" + str(widthofWeights*(i+1) + weightIndex) + "-1:" + str(widthofWeights*(i) + weightIndex) + "]"))
            # if merge_list[i] < 0:
                # print("    assign %s = (%s) * %s;" % (name,a,"weights[" + str(widthofWeights*(i+1) + weightIndex) + "-1:" + str(widthofWeights*(i) + weightIndex) + "]"))
        ...
        #BESPOKE ACCUMULATION WHEN WE KNOW THE WEIGHTS
        ...
        print("    //accumulate products")
        
        #get max width of sum
        #+1 is for the bias. This width is the possible accumulation result when we don't have binary tree addition. If we did, it could be less. The synthesis tool might realise and optimise this.
        swidth = pwidth + len(weights) - 1 + 1
        print("    wire signed [%d:0] %s;" % ((swidth-1),sumname))
        sum_str=") + $signed(".join(str(x) for x in prods)
        print("    assign %s = $signed(%s) + $signed(%s);" % (sumname, "biases[" + str(widthofBiases[layer] + biasIndex) + "-1:" + str(biasIndex) + "]", sum_str))
    ...
    def write_verilog_comb(self, fileHandle, sum_relu_size, neuron_masks=None, order=None, arg_masks=None):
        ...
        print("//generic design, we don't actually know the weights and intercepts, they are given to the network through the testbench.")
        print("//weights:", weights)
        print("//intercepts:", biases)
        
        numofWeights = len(self.coefs[0][0])*len(self.coefs[0])+len(self.coefs[1][0])*len(self.coefs[1])
        widthofWeights = sum(self.QW[0])
        numofBiases0 = len(self.intercept[0])
        widthofBiases0 = sum(self.QBshifted[0])
        numofBiases1 = len(self.intercept[1])
        widthofBiases1 = sum(self.QBshifted[1])
        
        print("module top ("+INPNAME+", weights, biases, "+OUTNAME+");")
        print("input ["+str(inp_num*width_a-1)+":0] " + INPNAME +";")
        print("input ["+str(numofWeights*widthofWeights-1)+":0] weights;")
        print("input ["+str(numofBiases0*widthofBiases0+numofBiases1*widthofBiases1-1)+":0] biases;")
        print("output ["+str(width_o-1)+":0] " + OUTNAME +";")
        print()
...
def generate_tb_comb(f, qmlp, period=200000000): #200ms by default
    ...
    bias_list = qmlp.interceptshifted
    ...
    print("    parameter NUM_W=" + str(len(qmlp.coefs[0][0])*len(qmlp.coefs[0])+len(qmlp.coefs[1][0])*len(qmlp.coefs[1])) + ";")
    print("    parameter WIDTH_W=" + str(sum(qmlp.QW[0])) + ";")
    print("    parameter NUM_B0=" + str(len(qmlp.intercept[0])) + ";")
    print("    parameter WIDTH_B0=" + str(sum(qmlp.QBshifted[0])) + ";")
    print("    parameter NUM_B1=" + str(len(qmlp.coefs[1])) + ";")
    print("    parameter WIDTH_B1=" + str(sum(qmlp.QBshifted[1])) + ";")
    print()
    ...
    weights_flat.reverse()
    
    wlength = sum(qmlp.QW[0])
    weight_str = ""
    for weight in weights_flat:
        if (weight < 0):
            temp = bin((2**wlength) + weight).split("b")[1]
            if (len(temp) < wlength):
                temp = "1" * (wlength - len(temp)) + temp
            weight_str = weight_str + str(wlength) + "'sb" + temp + ", "
        else:
            temp = bin(weight).split("b")[1]
            if (len(temp) < wlength):
                temp = "0" * (wlength - len(temp)) + temp
            weight_str = weight_str + str(wlength) + "'sb" + temp + ", "
    
    print("wire [NUM_W*WIDTH_W-1:0] weights;")
    print("//weights = " + str(weights_flat))
    print("assign weights = {" + weight_str[:-2] + "};")
    
    
    
    biases_flat = []
    blength = []
    for indlay, layer in enumerate(bias_list):
        for b in layer:
            biases_flat.append(b)
            blength.append(sum(qmlp.QBshifted[indlay]))
    biases_flat.reverse()
    blength.reverse()
    
    bias_str = ""
    for indbias, bias in enumerate(biases_flat):
        temp = bin(bias).split("b")[1]
        if (len(temp) < blength[indbias]):
            temp = "0" * (blength[indbias] - len(temp)) + temp
        if (bias < 0):
            temp = bin((2**blength[indbias]) + bias).split("b")[1]
            if (len(temp) < blength[indbias]):
                temp = "1" * (blength[indbias] - len(temp)) + temp
        bias_str = bias_str + str(blength[indbias]) + "'sb" + temp + ", "
    
    print("wire [(NUM_B0*WIDTH_B0)+(NUM_B1*WIDTH_B1)-1:0] biases;")
    print("//biases = " + str(biases_flat))
    print("assign biases = {" + bias_str[:-2] + "};")
    
    
    
    
    print('''top DUT(.inp(inp),
        .weights(weights),
        .biases(biases),
        .out(out)
        );
