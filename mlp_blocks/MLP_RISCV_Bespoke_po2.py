...
class MLP_RISCV_Bespoke_po2:
    def __init__(self, weights, biases, fxp_inputs, fxp_qrelu, fxp_weights, fxp_biases,
        ...
        self.QA = [(fxp_qrelu[0].s + fxp_qrelu[0].int, fxp_qrelu[0].frac), (fxp_qrelu[0].int + fxp_weights[1].int, fxp_qrelu[0].frac + fxp_weights[1].frac)]
    ...
    def write_asm_neuron(self, layer, neuron, inputs, outputreg):
        weights = [weight for weight in self.coefs[layer][neuron]]
        bias = self.intercept[layer][neuron]
        ...
        max_neg=0
        if bias != 0:
            shiftedBias=abs(bias)<<fixb
            if bias > 0:
                max_pos+=shiftedBias
            else:
                max_neg+=shiftedBias
        
        tag = "L" + str(layer) + "_N" + str(neuron)
        print(tag + ":")
        
        #check if all weights are non-positive and the bias is non-positive too, then just output 0
        if ((shiftedBias <= 0) and (max(weights) == 0)):
            print("	lui " + outputreg + ",0x00000				# all weights are non-positive, and the bias also is non-positive. we can just output 0.")
            return outputreg
        
        #put the bias into the sum register
        if (shiftedBias <= 1024):
            print("	addi tp,zero," + str(shiftedBias) + "			# x[tp] = " + str(shiftedBias) + " which is bias(" + str(bias) + ") but shifted to right position")
        elif (shiftedBias == 2048):
            print("	addi tp,zero,1024")
            print("	slli tp,tp,1				#bias which is 2048, impossible with addi because addi takes 12bit signed imm")
        else:
            if (shiftedBias < 0):
                print("	lui tp," + str(hex((shiftedBias + (1<<32)) % (1<<32))) + "				#bias which is " + str(shiftedBias))
            else:
                print("	lui tp," + str("0x{:08x}".format(shiftedBias)[0:-3]) + "				#bias which is " + str(shiftedBias))
        
        #now multiply and accumulate
        for (i, weight) in enumerate(weights):
            #skip mac if weight is 0
            if (weight == 0):
                continue
            
            if (layer == 0):
                print("	lw gp," + str(i*4) + "(ra)					# x[gp] = M[x[ra] + sext(" + str(i*4) + ")] load in" + str(i) + " to gp")
                print("	slli gp,gp," + str(int(log2(abs(weight)))) + "				# x[gp] = x[gp]*(" + str(abs(weight)) + ") (w_" + str(layer) + "_" + str(neuron) + "_" + str(i) + " = " + str(weight) + ")")
            else:
                print("	slli gp," + str(inputs[i]) + "," + str(int(log2(abs(weight)))) + "				# x[gp] = x[" + str(inputs[i]) + "]*(" + str(abs(weight)) + ") (w_" + str(layer) + "_" + str(neuron) + "_" + str(i) + " = " + str(weight) + ")")
            if ((layer == 1) and (i == len(weights)-1)):
                if (weight < 0):
                    print("	sub " + str(outputreg) + ",tp,gp				# out_l1_n" + str(neuron) + " = sum + prod" + str(i))
                else:
                    print("	add " + str(outputreg) + ",tp,gp				# out_l1_n" + str(neuron) + " = sum + prod" + str(i))
            else:
                if (weight < 0):
                    print("	sub tp,tp,gp				# sum = sum + prod" + str(i))
                else:
                    print("	add tp,tp,gp				# sum = sum + prod" + str(i))
        
        #if layer is 0, then we need to do qrelu activation
        if (layer == 0):
            print("	bge tp,zero,CHECK255_" + str(tag) + "				# if(tp >= 0) goto CHECK255")
            print("	lui " + str(outputreg) + ",0x00000				# out_l0_n" + str(neuron) + " = 0")
            if (neuron == len(self.coefs[0])-1):
                print("	jal gp,L1_N0					# go to next, we will disregard gp")
            else:
                print("	jal gp,L0_N" + str(neuron+1) + "					# go to next, we will disregard gp")
            print("CHECK255_" + str(tag) + ":")
            print("	srai " + outputreg + ",tp,4				# set " + outputreg + " = tp>>4 because we want specific bits") #TODO 4???
            print("	blt " + outputreg + ",sp,QUANTIZE_" + tag + "					# if(" + outputreg + " < sp) goto QUANTIZE")
            print("	add " + outputreg + ",zero,sp				# out_l0_n" + str(neuron) + "=255")
            if (neuron == len(self.coefs[0])-1):
                print("	jal gp,L1_N0					# go to next, we will disregard gp")
            else:
                print("	jal gp,L0_N" + str(neuron+1) + "					# go to next, we will disregard gp")
            print("QUANTIZE_" + str(tag) + ":")
            print("	srai " + outputreg + ",tp,4				# shift sum right by 4 bits because we want specific bits") #TODO 4???
        else:
            if (neuron == len(self.coefs[1])-1):
                print("	blt zero," + str(outputreg) + ",ARGMAX			# if(out_l1_n" + str(neuron) + " > 0) go to ARGMAX")
            else:
                print("	blt zero," + str(outputreg) + ",L1_N" + str(neuron+1) + "			# if(out_l1_n" + str(neuron) + " > 0) go to L1_N" + str(neuron+1))
            print("	lui " + str(outputreg) + ",0x00000				# out_l1_n" + str(neuron) + " = 0")
    ...
    def write_asm_argmax(self, inputs, outnames):
        print("ARGMAX:")
        print("	addi gp," + inputs[0].strip() + ",0				# out = " + outnames[0])
        print("	lui tp,0x00000				# outClass = 0")
        
        branches = []
        branches.append("IF-1") #unused, covering for simpler for loop
        for i in range(0, len(inputs)-2):
            branches.append("IF"+str(i))
        branches.append("PREDICTED")
        
        for i in range(1, len(inputs)):
            print("	bge gp," + inputs[i].strip() + "," + branches[i] + "				# if (out >= " + outnames[i] + ") goto " + branches[i])
            if (i < len(inputs)-1):
                print("	addi gp," + inputs[i].strip() + ",0")
                print("	addi tp,zero," + str(i))
            else:
                print("	addi gp," + inputs[i].strip() + ",0				# in the end, gp keeps the maximum value")
                print("	addi tp,zero," + str(i) + "				# and tp keeps the label of the value, which is the predicted outcome")
            print(branches[i] + ":")
        
        print("	sw tp," + str(len(self.coefs[0][0])*4) + "(ra)				# store the predicted result in the last memory address")
        print("	ebreak						# soft quit detected by simulator")
        print("	#jal gp,L0_N0				# go to L0_N0 for new prediction, we will disregard gp")
        print("")
        
        return 1
    
    def write_asm(self, fileHandle, dataset_name):
        stdoutbckp = sys.stdout
        sys.stdout = fileHandle
        
        print("#==========================================================================")
        print("#")
        print("#   The Healthcare PE Project")
        print("#")
        print("#   " + dataset_name + ".s: Based on the MLP model retrieved from the framework for " + dataset_name + " dataset")
        print("#")
        print("#   Gurol Saglam")
        print("#   CDNC - KIT")
        print("#")
        ...
        regnames = ["t0", "t1 ", "t2 ", "s0  ", "s1", "a0 ", "a1 ", "a2  ", "a3", "a4 ", "a5 ", "a6  ", "a7", "s2 ", "s3 ", "s4  ", "s5", "s6 ", "s7 ", "s8  ", "s9", "s10", "s11", "t3  ", "t4", "t5 ", "t6 "]
        
        outnames = []
        for i in range(0, len(self.coefs)):
            for j in range(0, len(self.coefs[i])):
                outnames.append("o_l" + str(i) + "_n" + str(j))
        
        print("# zero: 0		ra: inparr	sp : actlim	gp : prod")
        print("# tp  : sum		", end="")
        for i in range(1, len(outnames)+1):
            print(regnames[i-1] + ": " + outnames[i-1] + "	", end="")
            if ((i%4) == 3):
                print()
                print("# ", end="")
        for i in range(len(outnames)+1, len(regnames)+1):
            print(regnames[i-1] + ": " + "		", end="")
            if ((i%4) == 3):
                print()
                print("# ", end="")
        print()
        
        print("# ra=32bit, sp=8bits (value 255), prod=4+8=12bits")
        print("# sum=16,17,18 bits (python will analyze), o_l0_nX=8bits (qrelu), o_l1_nX=16,17,18 bits (python will analyze)")
        print()
        
        print(".section .data")
        print("	.align 2			# Align to word boundary (4 bytes)")
        for i in range(0, len(self.coefs[0][0])):
            print("	in" + str(i) + ":  .word 0		# Store the value  0 at address " + hex(2147549184+(i*4)))
        print()
        
        print(".section .text")
        print("	.align 2")
        print("	.globl _start")
        print("_start:                         # code entry point")
        print("	lui ra,0x80010				# pointer for start of input array, \"0x80010000\"")
        print("	addi sp,zero,255			# maxval = 255")
        inputs = []
        regnames2 = [temp for temp in regnames]
        for i in range(0, len(self.coefs)):
            for j in range(0, len(self.coefs[i])):
                outputreg = regnames.pop(0)
                inputs.append(self.write_asm_neuron(i, j, inputs, outputreg))
        
        for j in range(0, len(self.coefs[0])):
            inputs.pop(0)
            outnames.pop(0)
        
        self.write_asm_argmax(inputs, outnames)
        
        #writing finished, return stdout
        sys.stdout=stdoutbckp
