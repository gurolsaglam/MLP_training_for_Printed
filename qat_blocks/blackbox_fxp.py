...
from qkeras.quantizers import quantized_po2, quantized_relu, quantized_bits
...
def blackbox_int(fxp_model, weight_bias_size, relu_size, 
    ...
    hl_kq_maxv=weight_bias_size[0][0][1]
    # hl_bq_maxv=2**(weight_bias_size[0][1][1])
    hl_bq_maxv=weight_bias_size[0][1][1]
    hl_kreg=0.0001
    
    # ol_kq_maxv=2**(weight_bias_size[1][0][1])
    ol_kq_maxv=weight_bias_size[1][0][1]
    # ol_bq_maxv=2**(weight_bias_size[1][1][1])
    ol_bq_maxv=weight_bias_size[1][1][1]
    ol_kreg=0.0001
    ...
    model.add(QDense(hidden_layer, input_shape = (input_layer,), name = 'fc1', 
                kernel_quantizer = quantized_bits(weight_bias_size[0][0][0],
                                                # max_value=hl_kq_maxv,
                                                integer=hl_kq_maxv,
                                                use_stochastic_rounding=False,
                                                # quadratic_approximation=False,
                                                ),
                bias_quantizer = quantized_bits(weight_bias_size[0][1][0],
    ...
    model.add(QDense(output_layer, name = 'output',
                kernel_quantizer = quantized_bits(weight_bias_size[1][0][0],
                                                # max_value=ol_kq_maxv,
                                                integer=ol_kq_maxv,
                                                use_stochastic_rounding=False,
                                                # quadratic_approximation=False,
                                                ), 
                bias_quantizer = quantized_bits(weight_bias_size[1][1][0],
